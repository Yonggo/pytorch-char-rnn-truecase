...
class CharSplitLMMinibatchLoader(object):
    ...
    def text_to_tensor(in_textfile, out_vocabfile, out_tensorfile):
        ...
        print('loading data into tensor...')
        data = torch.LongTensor(length) # store it into 1D first, then rearrange
        pre_loading_percent = 0.0
        for pos, char in CharSplitLMMinibatchLoader.progressBar(rawdata, "Loading", "Complete", length=50):
            current_loading_percent = round(pos / float(length) * 100, 2)
            #if current_loading_percent > pre_loading_percent:
                #print("{}/{} => ".format(pos, length), end="")
                #print("loading: {}%".format(current_loading_percent), end="\r")
            ...
            pre_loading_percent = current_loading_percent

        print("loading complete: {} chars".format(length))
    ...
    def progressBar(iterable, prefix='', suffix='', decimals=1, length=100, fill='█', printEnd="\r", frequency=10):
        ...
        def printProgressBar(iteration):
            if iteration % frequency == 0 or iteration == total:
