...
# data
parser.add_argument('-data_dir', default='data/Dataset/deu_news_2018_10K-sentences',
...
parser.add_argument('-bi_directional', type=bool, default=False, help='if to be bi-directional')
# optimization
...
parser.add_argument('-checkpoint_dir', default='cv/dev', help='output directory where checkpoints get written')
...
if opt.gpuid >= 0 and torch.cuda.is_available():
    print('Using CUDA on GPU {}: {} ...'.format(opt.gpuid, torch.cuda.get_device_name()))
...
root_dir_checkpoint = "cv"
if not os.path.exists(root_dir_checkpoint):
    os.mkdir(root_dir_checkpoint)
...
if opt.init_from:
    ...
    protos.rnn = LSTM(vocab_size, opt.rnn_size, opt.num_layers, opt.dropout, opt.bi_directional)
    protos.criterion = nn.NLLLoss(reduction='mean')
    print('Models will be stored in:', opt.checkpoint_dir)
...
def val_split(split_index, max_batches=None):
    with torch.no_grad():
        print('evaluating loss over split index {}'.format(split_index), end="\r")
...
def calc_time_to_string(elapsed_time):
    time_kind = "s"
    temp_time = elapsed_time
    if temp_time >= 60:
        time_kind = "m"
        temp_time = temp_time / 60.0
    if temp_time >= 60:
        time_kind = "h"
        temp_time = temp_time / 60.0

    x_t = str(temp_time).split(".", 1)[0]
    y_t = str(temp_time).split(".", 1)[1]
    y_t = int(60 * (int(y_t[:2]) / 100)) if len(y_t) > 1 else int(60 * (int(y_t[:1]) / 10))

    if time_kind == "m":
        str_time = "{}m {}s".format(x_t, y_t)
    elif time_kind == "h":
        str_time = "{}h {}m".format(x_t, y_t)
    else:
        str_time = "{}s".format(x_t)
...
for i in range(1, iterations+1):
    ...
    if i % loader.ntrain == 0 and opt.learning_rate_decay < 1:
        ...
        if epoch >= opt.learning_rate_decay_after:
            ...
            decay_LR = 'decayed learning rate by a factor {} to {}'.format(decay_factor, lr)
            histroy_decayed_LR.append(decay_LR)
            print("\t" + decay_LR, end="\r")

    # every now and then or on last iteration
    if i % opt.eval_val_every == 0 or i == iterations:
        # evaluate loss on validation data
        val_loss = val_split(1) # 1 = validation
        val_losses.append(val_loss)

        if val_loss < min_val_loss:
            min_val_loss = val_loss
            savefile = '{}/lm_{}_epoch{:.2f}_{:.4f}.pt'.format(opt.checkpoint_dir, opt.savefile, epoch, val_loss)
            print('Saving checkpoint to {}'.format(savefile), end="\r")
    ...
    if i % opt.print_every == 0:
        ...
        progress = epoch/opt.max_epochs * 100
        remaining_time = calc_time_to_complete(actual_elapsed_time, progress)
        print(" Complete {:.1f}% | {} more : {}/{} (epoch {:.3f}), train_loss = {:6.8f}, time/batch = {:.4f}s".format(progress, remaining_time, i, iterations, epoch, train_loss, elapsed), end="\r")
...
print()
print("======================================")
print("Training time: {}".format(calc_time_to_string(total_elapsed)))
print("Total elapsed time: {}".format(calc_time_to_string(actual_elapsed_time)))
print("History decayed learning rate:")
print("\n".join(histroy_decayed_LR))
