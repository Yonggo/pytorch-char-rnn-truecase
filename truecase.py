...
parser.add_argument('-sent', type=bool, default=False, help='perform sentence-level truecasing')
parser.add_argument('-target', help='path of file to be predicted')
...
if opt.sent:
    ...
    print("Processing time: {:.2f}s".format(end - start))
    print("============== Result ==============")
    print(result)
    ...
    with open(opt.target, encoding='utf-8') as to_be_predicted:
        lines = to_be_predicted.read()
    end_latency_time_counter = time.time()
    gprint('latency time: {:.2f}s'.format(end_latency_time_counter - start_latency_time_counter))
    # truecase whole text at once
    result_prediction = beam_search_decoder('\n' + lines.rstrip(), opt.beamsize)
