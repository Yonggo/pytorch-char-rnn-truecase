...
parser.add_argument('-target', required=True, help='path of file to be predicted')
parser.add_argument('-result', required=True, help='path of file predicted')
...
if opt.gpuid >= 0 and torch.cuda.is_available():
    gprint('using CUDA on GPU {}: {} ...'.format(opt.gpuid, torch.cuda.get_device_name()))
...
if opt.sent:
    ...
    gprint('latency time: {:.2f}s'.format(end_latency_time_counter - start_latency_time_counter))
    for line in tqdm(lines, desc="num lines"):
        # truecase line by line
        print(beam_search_decoder('\n' + line.rstrip(), opt.beamsize))
else:
    gprint('performing document-level truecasing...')
    gprint('memory is carried over to the next line')

    print("Loading data...")
    with open(opt.target, encoding='utf-8') as to_be_predicted:
        lines = to_be_predicted.readlines()
    ...
    end_latency_time_counter = time.time()
    gprint('latency time: {:.2f}s'.format(end_latency_time_counter - start_latency_time_counter))
    for sentence in tqdm(sentences, desc="Number of lines"):
        result_prediction.append(beam_search_decoder('\n' + sentence.rstrip(), opt.beamsize, False))

    if len(result_prediction) != len(labels):
        raise Exception("Size result of prediction is not same size of labels")

    with open(opt.result, 'w', encoding='utf-8') as f:
        for idx in len(result_prediction):
            f.write(labels[idx] + " " + result_prediction[idx] + "\n")
        f.close()
